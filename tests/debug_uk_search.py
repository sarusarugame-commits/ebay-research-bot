...
params = {"q": "CASIO Oceanus OCW-S6000", "limit": 10, "filter": "conditions:{NEW}"}
r = requests.get(search_url, headers=headers, params=params).json()

for it in r.get("itemSummaries", []):
    print(f"Item: {it.get('title')[:40]}")
    print(f"  Price: {it.get('price')}")
    print(f"  Shipping: {json.dumps(it.get('shippingOptions', []), indent=2)}")
    print("-" * 50)
