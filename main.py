...
def print_token_stats():
    ...
    # 料金設定 ($/1M tokens) - Qwen 3.5-Flash 定価
    IN_PRICE = 0.10
    OUT_PRICE = 0.40
...
def execute_research_session(url, browser):
    ...
    try:
        ...
        if final_name and final_name != "特定不能":
            ...
            # ブラウザ系（BAN対策で3並列上限）
            browser_tasks = [
                ("メルカリ",       lambda: search_mercari(search_query, get_fresh_browser(), max_results=15)),
                ("ラクマ",         lambda: search_rakuma(search_query,  get_fresh_browser(), max_results=10)),
                ("駿河屋",         lambda: search_surugaya(search_query, get_fresh_browser(), max_results=10)),
            ]
            # API系（制限緩め、2並列）
            api_tasks = [
        ...
        if best_item:
            ...
            if token:
                ...
                def _search_market(market_id):
                    ...
                    b_used = get_fresh_browser() # 並列処理用に独立したブラウザを生成
                    try:
                        top3 = process_market(token, market_id, final_en_name, img_url, target_cond, model_number=ebay_model_number, exclude_id=item_id, ebay_title=target_item.get('title', ''), base_thresholds=domestic_thresholds, browser=b_used)
                        if target_cond == "NEW" and not top3:
                            print(f"    [!] {label}で新品が見つからないため、中古で再検索します...")
                            top3 = process_market(token, market_id, final_en_name, img_url, "USED", model_number=ebay_model_number, exclude_id=item_id, ebay_title=target_item.get('title', ''), base_thresholds=domestic_thresholds, browser=b_used)
        ...
        print(f"[!] execute_research_session エラー: {e}")
        traceback.print_exc()
