...
def estimate_weight_with_llm(ebay_img_url, final_name):
    ...
    print(f"[*] AI ({QWEN_MODEL}) が画像から重量・サイズを推論しています...")
    # Qwen (OpenRouter) を優先
    ...
    print(f"    [*] Qwen失敗 → Gemini ({GEMINI_MODEL}) でリトライします...")
...
def analyze_item_safety_and_tariff(ebay_img_url, all_img_urls=None):
    ...
    print(f"[*] AI ({QWEN_MODEL}) が {len(imgs_to_use)} 枚の画像から安全性と最良画像を判定しています...")
...
def judge_similarity_with_llm(ebay_img_url, scraped_items):
    ...
    def _judge_one(item):
        ...
        score = 0
        print(f"  -> 画像比較中 ({QWEN_MODEL}): {mercari_img_url[:60]}...")
...
- 背景、照明、撮影角度、付属品や箱の有無などは無視してください。
- 本体の形状、テクスチャ、ロゴの配置、文字盤のデザインなど「物理的な製品特徴」が一致しているかを厳格に見てください。
- 同一製品의別カラーバリエーションや世代違いは "match": false としてください。
